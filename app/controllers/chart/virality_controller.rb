class Chart::ViralityController < HighchartsController
  ...
  def unique_nonreg_emails
    data_src = RollupData::UniversalDatasource.new(
      :period => (2.months.ago..DateTime.now),
      :span => params[:span],
      :queries_to_fetch => %w(share.email.unique_non_users.mean share.email.unique_non_users.median),
      :calculate_now => true
    )

    #Divide each value by 100
    data_src.chart_series.each_index do |i|
      data_src.chart_series[i][:data].map!{|val| val / 100.0 }
    end
    ...
    respond_to do |wants|
      ...
      wants.json do
        render :json => {
          ...
          :title => {
            :text => "Mean and Median # unique non-reg users per share"
          },
          :subtitle => {
            :text => data_src.chart_subtitle
          },

...
protected
  ...
  def send_xls(series, categories = nil)
    ...
    worksheet.row(0).default_format = RollupData::CohortsDatasource::XLS_FORMAT[:series_names]
    worksheet.column(0).default_format = RollupData::CohortsDatasource::XLS_FORMAT[:categories]
    ...
    header.each_with_index do |cat, cat_idx|
      ...
      data.each_with_index do |serie, serie_idx|
        ...
        worksheet.row(1+cat_idx).set_format(1+serie_idx, RollupData::CohortsDatasource::XLS_CELL_TYPE[:percent])
      end
    end

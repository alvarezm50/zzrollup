class KFactorSweep < BaseSweep
  ...
  def self.calc_k_value(days_count, set_type)
    ...
    completed_invitations_sent_query = <<-SQL
      select count(distinct invitations.id) from invitations, tracked_links
        where invitations.tracked_link_id = tracked_links.id
          ...
          and tracked_links.user_id in (select id from users where #{time_field} > DATE_ADD(NOW(), INTERVAL -#{days_count} DAY))
    SQL
